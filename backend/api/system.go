...
var monitoredServices = []string{"nginx", "mysql", "sshd", "postfix", "fail2ban", "cron", "ufw"}
...
// chi_urlParam is a thin wrapper so we don't import chi in every file.
// Replace with chi.URLParam(r, key) when chi is available.
func chi_urlParam(r *http.Request, key string) string {
	// In production with chi router this would be: return chi.URLParam(r, key)
	// This stub reads from query params for testing without chi dependency.
