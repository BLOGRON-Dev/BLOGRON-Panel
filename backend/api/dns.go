...
func CreateDNSZone(w http.ResponseWriter, r *http.Request) {
	...
	// Reload BIND
	if _, err := util.RunCmd("systemctl", "reload", "bind9"); err != nil {
		util.WriteError(w, http.StatusInternalServerError, "bind9 reload failed: "+err.Error())
		return
	}
...
func DeleteDNSZone(w http.ResponseWriter, r *http.Request) {
	...
	util.RunCmd("systemctl", "reload", "bind9")
